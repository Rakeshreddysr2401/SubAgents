#!/usr/bin/env python3
"""Swiggy MCP login — OAuth 2.1 PKCE, writes ~/.subagents/mcp_tokens.json.

Run wherever a browser can reach this machine's localhost:
  desktop:        uv run python scripts/swiggy_login.py
  headless box:   ssh -L 8976:localhost:8976 <server>   (from your laptop)
                  uv run python scripts/swiggy_login.py --no-browser   (on the box)
                  → open the printed URL in the laptop browser; the localhost
                    redirect tunnels back to the server's callback listener.

The access token lasts ~5 days and there is no refresh flow — when it expires
the app degrades gracefully and pushes a browser notification to re-run this
script. A running server picks up the new token automatically on the next chat
turn (header hot-reload); only a NEVER-configured server needs one restart
after the first login.

Standalone on purpose: stdlib + httpx only, never imports src/.
"""
...
def register_client(endpoints: dict, redirect_uri: str, existing: str | None) -> str:
    ...
    r = httpx.post(endpoints["registration_endpoint"], json={
        "client_name": "SubAgents assistant",
...
def main() -> None:
    ...
    ap.add_argument("--token-file",
                    default=os.getenv("SUBAGENTS_MCP_TOKENS", "~/.subagents/mcp_tokens.json"))
    ...
    if not opened:
        print("\nOpen this URL in a browser (headless server: first run "
              f"`ssh -L {args.port}:localhost:{args.port} <server>` from your "
    ...
    print("   A running server picks this up automatically on the next chat "
          "turn. If Swiggy was never configured before, restart the server "
          "once so the agents get their MCP tools.")
