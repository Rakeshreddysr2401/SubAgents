...
| Delivery workflow | Tested end-to-end: order placed → background tracking → door navigation |
...
```
  ...
  │    YOLOv8s via NITROS zero-copy          │
  │    RealSense D555 depth+RGB+IMU [soon]   │
  │                                          │
  │  Container 2 — ai_stack (jetson build)   │
  │    openWakeWord → Whisper STT            │
  │    Moondream VLM (MLC INT4, 0.8 GB)     │
  │    Kokoro TTS → Speaker                  │
  └──────────────┬───────────────────────────┘
                 │  ROS2 Jazzy topics (local network)
                 ▼
  ┌──────────────────────────────────────────┐
  │  RASPBERRY PI 5  ·  BODY                 │
  │  LangGraph Supervisor (routes only)      │
  │    ├── Conversation  ├── Vision          │
  │    ├── Navigation    ├── Status          │
  │    ├── Food ─────────────► [Swiggy API] │
  │    └── Delivery Tracker                  │
  ...
  └──────────────────────────────────────────┘
  ┌──────────────────────────────────────────┐
  │  MAC MINI  ·  LLM server (local network) │
  │  llama.cpp · any GGUF model              │
  │  → NVIDIA NIM target for hackathon       │
  └──────────────────────────────────────────┘
```

All inter-device communication: local WiFi.
External internet: Swiggy API only.
...
| Food ordering + delivery workflow | Working end-to-end |
...
For reference: commercial robots with comparable sensing and navigation
capabilities typically cost ₹5,00,000 and above. The cost difference comes
primarily from running inference locally rather than paying for cloud compute.
...
| Month 1 | SLAM room auto-labeling | Robot asks room name on first visit, stores locally |
| Month 2 | Memory agent | Remembers user name, preferences, room vocabulary |
...
| Month 4–6 | Security agent | Local face recognition, visitor detection |
| Month 6–12 | Healthcare agent | Medication reminders, fall detection, emergency alerts |

Each item above is a new specialist agent. The supervisor routing, hardware
stack, and inter-device communication remain unchanged for all of them.
...
| Limitation | Plan |
|------------|------|
| SLAM untested live | Test immediately on D555 arrival, document results |
| Mac Mini dependency | Small GGUF on Jetson as fallback — Month 1 priority |
| No persistent memory | Memory agent with local vector store — Month 2 |
| Manual SLAM map setup | Auto-labeling on first visit — Month 1 |
| No manipulation | Robotic arm as new BASE module over ROS2 — future work |
